<script lang="ts">
	...
	// ContextMenus
	import { contextMenu, type ContextMenuSettings } from '@skeletonlabs/skeleton';
	...
	const settings: DocsShellSettings = {
		...
		name: 'Context Menus',
		description:
			'Create context menus using the power of <a class="anchor" href="https://floating-ui.com/" target="_blank" rel="noreferrer">Floating UI</a>.',
		imports: ['contextMenu'],
		types: ['ContextMenuSettings'],
		stylesheetIncludes: ['all', 'elements'],
		stylesheets: ['elements/contextMenus'],
		source: 'utilities/ContextMenu',
		aria: 'https://www.w3.org/WAI/ARIA/apg/patterns/menu/',
		classes: [
			['<code class="code">[data-context-menu]</code>', '', `Follows Floating UI's best practices. Sets hidden by default.`],
		],
		parameters: [
			['<code class="code">target</code>', 'string', '-', '-', 'Match the popup data value of <code class="code">[data-context-menu]</code>'],
			['<code class="code">placement</code>', 'string', '-', 'bottom-start', 'Set the placement position.'],
			['<code class="code">closeQuery</code>', 'string', 'a[href], button', '-', 'Query list of elements that will close the context menu.'],
		...
		keyboard: [
			['<kbd class="kbd">Tab</kbd>', 'Focus the next focusable item in menu context.'],
			['<kbd class="kbd">Shift + Tab</kbd>', 'Focus the previous focusable item in menu context.'],
			['<kbd class="kbd">Esc</kbd>', 'Closes all open context menus at once.']
		],
